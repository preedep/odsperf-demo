...
echo -e "   • Collection: ${MAGENTA}account_statements${NC}"
echo ""

# Confirm before running
echo -e "${YELLOW}⚠️  This test will:${NC}"
echo -e "   1. Drop the existing 'account_statements' collection (if any)"
echo -e "   2. Create 10 hot account documents"
echo -e "   3. Perform 10,000 write operations (1,000 writes × 10 accounts)"
echo -e "   4. Append 10 statements per write (100,000 total statements)"
echo ""
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo "     mongosh \"$MONGODB_URI\" --eval 'db.account_statements.findOne()'"
    echo ""
    echo "  2. Check document sizes:"
    echo "     mongosh \"$MONGODB_URI\" --eval 'db.account_statements.stats()'"
    echo ""
